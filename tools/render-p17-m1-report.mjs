...
for (const candidate of candidates) {
  ...
  try {
    const output = execFileSync(candidate.command, [...candidate.prefix, '-c', probe], {
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore'],
      windowsHide: true,
    });
    const actual = JSON.parse(output);
    if (Object.entries(requiredVersions).some(([name, version]) => actual[name] !== version)) continue;
    runtime = candidate;
    break;
  } catch {
...
if (!runtime) {
  process.stderr.write(
    'No Python runtime with the pinned P17-M1 report dependencies was found. '
      + 'Install verification/benchmarks/strix21/reporting/requirements-p17-m1.txt '
      + 'or set P17_PYTHON.\n',
  );
  process.exit(1);
}
