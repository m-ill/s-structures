...
try{
 ...
 assert.ok(torsion.requiredAt>0,JSON.stringify(torsion));assert.equal(torsion.strengthStatus,'OK',JSON.stringify(torsion));assert.equal(torsion.status,'NOT_CHECKED');assert.equal(torsion.codeBasis.status,'NOT_ESTABLISHED');
 ...
 console.log('PASS WebMCP typed torsion input -> elastic workflow -> shared-area calculations -> paged result with remaining detailing gate');
