...
assert.ok(inputState.numericFields.some((item) => item.id === 'accidentalEccentricityRatio'));
...
const accidentalEccentricityRatio = document.getElementById('ssLoadBasisAccEcc');
...
assert.ok(accidentalEccentricityRatio);
...
accidentalEccentricityRatio.value = '0.08';
...
assert.equal(model.designBasis.accidentalEccentricityRatio, 0.08);
