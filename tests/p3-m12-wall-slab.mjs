...
assert.equal(trace.contract.milestone, 'P3-M12');
assert.deepEqual(trace.contract.tickets, ['P3-T73', 'P3-T74', 'P3-T75']);
...
assert.equal(trace.contract.featureTicketMap.wallMidPier, 'P3-T73');
assert.equal(trace.contract.featureTicketMap.shellV1, 'P3-T74');
assert.equal(trace.contract.featureTicketMap.semiRigidDiaphragm, 'P3-T75');
...
assert.equal(trace.summary.ticketCoverage.find((row) => row.ticket === 'P3-T73').covered, true);
assert.equal(trace.summary.ticketCoverage.find((row) => row.ticket === 'P3-T74').covered, false);
assert.equal(trace.summary.ticketCoverage.find((row) => row.ticket === 'P3-T75').covered, false);
assert.equal(trace.review.traceReady, false);
assert.equal(trace.review.coverageComplete, false);
assert.equal(trace.review.coverageReviewRequired, true);
assert.equal(trace.review.wallMidPierReady, true);
assert.equal(trace.review.shellFrameLinkReady, false);
assert.equal(trace.review.semiRigidRedistributionReady, false);
assert.equal(trace.review.preliminarySolverTreatment, true);
assert.equal(trace.review.engineerReviewRequired, true);
assert.equal(trace.review.productionReady, false);
assert.deepEqual(trace.review.uncoveredTickets, ['P3-T74', 'P3-T75']);
assert.ok(trace.review.blockers.includes('uncovered-P3-T74'));
assert.ok(trace.review.blockers.includes('uncovered-P3-T75'));
assert.equal(trace.review.agentDecision, 'complete-wall-slab-ticket-coverage-before-review');
...
assert.equal(trace.wallMidPier.rows[0].cantileverHandcalc.status, 'available-preliminary');
assert.equal(trace.wallMidPier.rows[0].cantileverHandcalc.method, 'V*h^3/(3*E*I)');
assert.ok(Number.isFinite(trace.wallMidPier.rows[0].cantileverHandcalc.lateralStiffness));
assert.equal(trace.wallMidPier.rows[0].cantileverHandcalc.review, 'cantilever-wall-handcalc-preliminary');
const wallTraceWithoutAnalysis = buildWallSlabEquivalentTrace(wallModel, null);
assert.equal(wallTraceWithoutAnalysis.wallMidPier.rows[0].cantileverHandcalc.status, 'force-recovery-required');
...
assert.equal(shellFrameResult.byCombo.SLS1.shellFrameAssembly.linkCount, 6);
assert.equal(shellFrameResult.byCombo.SLS1.memberResults[shellAssembly.members[0].id], undefined);
...
assert.ok(shellFrameTrace.review.uncoveredTickets.includes('P3-T73'));
...
assert.equal(softResult.byCombo.SLS1.memberResults[softExpansion.members[0].id], undefined);
assert.ok(stiffResult.byCombo.SLS1.disp.D[0] > softResult.byCombo.SLS1.disp.D[0]);
...
assert.ok(slabTrace.review.uncoveredTickets.includes('P3-T73'));
