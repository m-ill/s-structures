...
try{
 ...
 const record=m.designDetails.connections[0],beam=m.designDetails.reinforcement.find(r=>r.id==='RB'),original=beam.bars,left=m.designDetails.reinforcement.find(r=>r.id==='RL');
 ...
 for(const y of [-.105,-.09,-.07,-.05,.05,.07,.09,.105])for(const z of [-.105,-.075,-.045,.045,.075,.105]){
  beam.bars=[{...original[0],y,z}];left.bars=structuredClone(beam.bars);const paths=evaluateJointCongestion(m,record);
 ...
 mkdirSync('output/phase25',{recursive:true});writeFileSync('output/phase25/joint-through-layout-probe.json',JSON.stringify({scope:'48 single bar geometry probes; combined arrangement still requires all checks',rows},null,2));
