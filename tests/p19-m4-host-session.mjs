...
const host=installHostWebMcp({window:win,iframe,projectId:'A'});load();assert.equal(definitions.length,4);
const get=definitions.find(x=>x.name==='read_sstructures_tool');assert.equal((await get.execute({name:'get_workflow_context',arguments:{}})).ok,true);
assert.throws(()=>child.call({project:'A',session:child.session,nonce:'forged'},'get_workflow_context',{}),{code:'SESSION_BINDING_INVALID'});
frame.location.origin='https://evil.example';await assert.rejects(get.execute({name:'get_workflow_context',arguments:{}}),{code:'SESSION_BINDING_INVALID'});frame.location.origin=win.location.origin;
const doc=frame.document;frame.document={};await assert.rejects(get.execute({name:'get_workflow_context',arguments:{}}),{code:'SESSION_BINDING_INVALID'});frame.document=doc;
iframe.contentWindow={};await assert.rejects(get.execute({name:'get_workflow_context',arguments:{}}),{code:'SESSION_BINDING_INVALID'});iframe.contentWindow=frame;
child.project='B';await assert.rejects(get.execute({name:'get_workflow_context',arguments:{}}),{code:'SESSION_BINDING_INVALID'});child.project='A';
host.dispose();assert.equal(child.active,false);assert.throws(()=>get.execute({name:'get_workflow_context',arguments:{}}),/inactive/);
