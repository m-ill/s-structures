...
// Product path: the schedule reports the provided practical RC inputs and does
// not infer bars from preliminary ratios, so an unreinforced member stays open.
const preparedDetail = buildRcDetailingReport(beam, beamAnalysis);
assert.equal(preparedDetail.version, RC_PROVIDED_SCHEDULE_VERSION);
assert.equal(preparedDetail.basis, 'provided-practical-checks');
assert.equal(preparedDetail.designTransferAllowed, false);
assert.equal(preparedDetail.rows[0].status, 'NOT_CHECKED');

// Legacy preliminary schedule stays covered through the legacy design payload.
const legacyAnalysis = { ...beamAnalysis, design: evaluateLegacyDesign(beam, beamAnalysis) };
const beamDetail = buildRcDetailingReport(beam, legacyAnalysis);
...
assert.equal(beamReport.data.rcDetailing.version, RC_PROVIDED_SCHEDULE_VERSION);
...
const columnDetail = buildRcDetailingReport(column, { ...columnAnalysis, design: evaluateLegacyDesign(column, columnAnalysis) });
...
assert.equal(agentDetail.version, RC_PROVIDED_SCHEDULE_VERSION);
