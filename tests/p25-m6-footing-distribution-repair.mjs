...
try{
 ...
 const before=ctx.bridge.getPracticalDesignSnapshot(evaluation.evaluationId),flex=before.checks.find(c=>c.checkId==='foundation-flexure');assert.equal(before.checks.find(c=>c.checkId==='foundation-distribution').status,'NG');
 ...
 const stored=(await ctx.call('get_design_records',{channel:'foundations',id:'F'})).rows[0];assert.equal(stored.aggregateMaxSize,.02);assert.equal(after.checks.find(c=>c.checkId==='foundation-spacing').status,'OK');assert.equal(stored.thickness,.3);assert.equal(stored.barDistribution,'kds-centered-band');assert.equal(after.checks.find(c=>c.checkId==='foundation-distribution').status,'OK');assert.equal(stored.reinforcement.bottomB.diameter,.016);assert.equal(stored.reinforcement.bottomL.diameter,.016);
 console.log('PASS WebMCP rectangular footing distribution NG -> centered-band and steel repair -> same-owner distribution, strength and spacing OK');
