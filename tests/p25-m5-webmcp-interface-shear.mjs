...
try{
 ...
 assert.equal(first.status,'NG',JSON.stringify(first));assert.equal(first.reason,'COLUMN_INTERFACE_TENSION_DEVELOPMENT_INSUFFICIENT');assert.ok(first.interfaceShear);assert.equal(first.interfaceShear.mu,1);assert.equal(first.development[0].mode,'tension-for-interface-shear');
 ...
 assert.equal(after.status,'OK',JSON.stringify(after));assert.equal(after.providedBelow,.65);assert.equal(after.providedAbove,.65);assert.equal(after.interfaceShear.permanentCompressionCredit,0);assert.ok(after.codeReferences.some(r=>r.id==='142022'&&r.clause.includes('4.6')));
 ...
 const quantityVolume=drawing.quantities.filter(q=>q.kind==='foundation-bar').reduce((n,q)=>n+q.count*q.area*q.bodyLength,0)+drawing.quantities.filter(q=>q.kind==='column-bar-extension').reduce((n,q)=>n+q.bodyVolume,0);
 assert.ok(Math.abs(job.best.objective.value-quantityVolume)<1e-12,'candidate quantity includes the same foundation layers and column embedment as drawings');
 console.log('PASS actual WebMCP compression/shear interface, tension-development NG, explicit anchorage/thickness candidate and apply/review OK');
