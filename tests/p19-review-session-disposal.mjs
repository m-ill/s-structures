...
let finish,activities=0;
const pendingTools=createWebMcpTools({agent,bridge:{},setView:()=>new Promise(resolve=>{finish=resolve;}),onActivity:()=>activities++});
const pending=pendingTools.find(t=>t.name==='set_workspace_view').execute({view:'modeling'});
pendingTools.dispose();finish({ok:true});
await assert.rejects(pending,{code:'SESSION_DISPOSED'});
assert.equal(activities,0,'Do not publish activity or results after the session has ended');
