...
import {
  buildPhase9M7Evidence,
  upgradePhase9ManifestToM7,
  validatePhase9M7Evidence,
  validatePhase9M7Manifest,
} from '../../src/compute/governance/phase9M7.js';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..');
const EVIDENCE_PATH = path.join(ROOT, 'verification', 'evidence', 'validation', 'phase9', 'p9-m7-nonlinear-batch.json');
const MANIFEST_PATH = path.join(ROOT, 'verification', 'specs', 'phase9', 'release-manifest.json');
const PRIOR_PATH = path.join(ROOT, 'verification', 'evidence', 'validation', 'phase9', 'p9-m6-sparse-eigen.json');
...
const functional = runTest('tests/p9-m7-nonlinear-batch.mjs');
const generatedAt = new Date().toISOString();
const sourceRevision = detectSourceRevision();
const evidence = buildPhase9M7Evidence({ generatedAt, sourceRevision, priorEvidenceHash: prior.artifactHash, functional });
assertValid(validatePhase9M7Evidence(evidence), 'M7 evidence');
const manifest = upgradePhase9ManifestToM7(previousManifest, evidence, { generatedAt, sourceRevision });
assertValid(validatePhase9M7Manifest(manifest), 'M7 release manifest');
