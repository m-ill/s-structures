...
import { createBenchmarkModel } from '../../../src/examples/verification.js';
...
function runBm05TrussRotations() {
  ...
  const analysis = analyzeModel(model);
  const solver = analysis.byCombo?.D_ONLY?.solver || {};
  const fixed = solver.autoFixedDofs || [];
  const stabilized = solver.stabilizedRotationDofs || [];
  const rotations = [...new Set([...fixed, ...stabilized])].filter((label) => /^N2\.r[xyz]$/.test(label));
  ...
  return diagnosticObservation(
    ...
    { autoFixedDofs: fixed, stabilizedRotationDofs: stabilized, analysisOk: analysis.ok },
    model,
  );
}
