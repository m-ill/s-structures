...
const tests = (await readdir('tests'))
  .map((file) => {
    const match = /^p13-m(\d+)-.+\.mjs$/i.exec(file);
...
if (!tests.length) throw new Error('No Phase 13 tests matched the requested range.');
if (listOnly) {
  for (const test of tests) console.log(`P13-M${test.milestone}\t${test.file}`);
  process.exit(0);
}
for (const test of tests) {
  console.log(`\n[P13-M${test.milestone}] ${test.file}`);
...
function parseBounds(values) {
  ...
  for (const value of values) {
    ...
    else if (/^(?:P13-)?M?\d+$/i.test(value)) {
      const match = /(?:^|M)(\d+)$/i.exec(value);
      from = to = Number(match[1]);
    }
    else throw new Error(`Unsupported Phase 13 test argument: ${value}`);
  }
  if (!Number.isInteger(from) || !Number.isInteger(to) || from < 0 || to > 9 || from > to) {
    throw new Error('Invalid Phase 13 range.');
  }
  return { from, to };
}
