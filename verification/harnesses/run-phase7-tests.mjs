...
const tests = (await readdir('tests'))
  .map((file) => {
    const match = /^p7-m(\d+)-.+\.mjs$/i.exec(file);
...
if (!tests.length) throw new Error('No Phase 7 tests matched the requested milestone range.');

if (listOnly) {
  for (const test of tests) console.log(`P7-M${test.milestone}\t${test.file}`);
  process.exit(0);
}

for (const test of tests) {
  console.log(`\n[P7-M${test.milestone}] ${test.file}`);
...
function parseBounds(values) {
  ...
  for (const value of values) {
    ...
    else if (/^(?:P7-)?M?\d+$/i.test(value)) from = to = parseMilestone(value);
    else throw new Error(`Unsupported Phase 7 test argument: ${value}`);
  }
  if (from > to) throw new Error(`Invalid Phase 7 milestone range: M${from}..M${to}`);
  return { from, to };
}

function parseMilestone(value) {
  const match = /^(?:P7-)?M?(\d+)$/i.exec(String(value).trim());
  if (!match) throw new Error(`Invalid Phase 7 milestone: ${value}`);
