...
const tests = (await readdir('tests'))
  .map((file) => {
    const match = /^p14-m(\d+)-.+\.mjs$/i.exec(file);
...
if (!tests.length) throw new Error('No Phase 14 tests matched the requested range.');
if (listOnly) {
  for (const test of tests) console.log(`P14-M${test.milestone}\t${test.file}`);
  process.exit(0);
}
for (const test of tests) {
  console.log(`\n[P14-M${test.milestone}] ${test.file}`);
...
function parseBounds(values) {
  ...
  for (const value of values) {
    ...
    else if (/^(?:P14-)?M?\d+$/i.test(value)) {
      const match = /(?:^|M)(\d+)$/i.exec(value);
      from = to = Number(match[1]);
    } else throw new Error(`Unsupported Phase 14 test argument: ${value}`);
  }
  if (!Number.isInteger(from) || !Number.isInteger(to) || from < 0 || to > 11 || from > to) {
    throw new Error('Invalid Phase 14 range.');
  }
  return { from, to };
}
