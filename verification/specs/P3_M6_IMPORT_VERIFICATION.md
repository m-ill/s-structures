...
2026-07-03 ImportCandidate identity hardening: `validateImportCandidate()` now rejects duplicate node IDs, missing member IDs, duplicate member IDs, and self-loop members. `tests/p3-import-geometry.mjs` locks these cases so AI agents and review UI can rely on stable node/member identifiers when inspecting or resolving drawing imports.
