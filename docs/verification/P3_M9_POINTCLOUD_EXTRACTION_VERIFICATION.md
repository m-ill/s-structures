...
2026-07-03 owner-review readiness update: `buildPointCloudExtractionReview()` no longer marks a benchmark result as production-ready when `realScanValidation` is simply `checked`. It now exposes `ownerReviewReady` for the synthetic-pass plus checked-real-scan state while keeping `productionReady: false`; final production use remains controlled by the separate point-cloud validation and launch-readiness gates.

2026-07-03 evidence-coverage update: `getPhase3PointCloudValidationReview()` now exposes `evidenceCoverage` for accepted project evidence rows such as owner point-cloud files, large-file performance records, and real-scan extraction validation. Evidence coverage is shown to AI agents while real-scan rows, benchmark rows, and owner review remain separate gates.

2026-07-03 real-scan limitation wording update: point-cloud extraction limitations now distinguish `real-field-pointcloud-validation-pending` from `real-field-pointcloud-owner-review-required`. A `checked` real-scan flag can remove the pending wording, but `productionReady` remains false and owner review remains required.
