...
기준일 2026-09-13 · 기준 커밋 `a2675a62` · 상태: **COMPLETE — M0~M6 완료 / 게이트 확장됨**
...
M5에서 등록 항목 2건이 추가돼 전체 35건이 됐다. 둘 다 원래 조사가 놓쳤고 게이트 확장이 찾아낸 것이다.
...
| M5 게이트 확장 | **DONE** | phase 16~25의 445건이 `npm test`에 편입됐다. 필수 432건 통과, 제외 13건, 12.9분 |
| M6 문서 갱신 | **DONE** | `docs/README.md` 현재 개발이 Phase26, 폴더 지도에 phase26·phase27 추가 |
...
### M5 — 정식 게이트 확장 (2026-09-14)

`verification/harnesses/run-phase16-25-tests.mjs`를 만들고 `npm test` 사슬 끝에 붙였다. phase 16~25의 **445건**이 이제 정식 게이트에서 실행된다(기존 게이트가 돌던 phase 7~15는 251건이었다).
...
시간이 감당 가능해 사슬을 나누지 않았다.

**제외 방식**

- 제외 목록은 [대장](DEBT_REGISTER.json) **한 곳**에서만 결정된다. 러너는 검사 이름을 갖고 있지 않다.
- 실행할 때마다 제외 건수와 **각 항목의 사유**를 출력한다.
- 제외된 검사도 **실행한다.** 통과하기 시작하면 게이트가 빨개지며 "대장에서 승격하라"고 실패한다. 실제로 통과 중인 검사 1건을 임시로 `DEFERRED`로 표시해 이 동작을 확인했고 대장을 원복했다. 제외가 영구 은폐가 되지 않는다.
- 제외된 검사의 실패 출력은 조용하다. 예상된 실패이기 때문이며, 예상치 못한 실패만 출력한다.

**게이트 확장이 즉시 찾아낸 것 2건** — 원래 조사(영향 범위 266건)가 놓쳤던 항목이다.

- **P26-S08 해결**: `memberReinforcementProposal`에 `model` 인자가 추가됐는데 `p25-m6-longitudinal-proposal`만 2개 인자로 호출해 `TypeError`로 죽었다. 제품 호출부와 형제 시험은 3개를 넘긴다. 시험이 낡은 것이라 인자를 채웠다.
- **P26-S07 `DEFERRED`**: `p18-benchmark-engine-report-package`가 **저장소 밖** 납품물(`자료/testreport`)을 검사한다. 폴더 정리로 끊긴 경로는 `resolveTestreportRoot()`로 고쳤지만, 디스크의 보고서 목차 구조가 `tools/`의 생성기와 다르다. 재생성하면 오너 산출물 21건을 덮어쓰므로 내용 계약은 오너 판단으로 넘긴다. 깨끗한 체크아웃에는 이 자료가 없다는 점도 함께 기록한다.

### M6 — 문서·릴리스 게이트 갱신 (2026-09-14)

`docs/README.md`의 현재 개발을 Phase26으로 바꾸고 폴더 지도에 `phase26/`·`phase27/`을 넣었다. 문서 참조 무결성과 문서 커버리지 검사 통과.
...
`DEFERRED` 15건의 내역: RC 미구현 11(Phase27), 봉인 lock 재발행 대기 1, 저장소 밖 납품물 검사 1, 호환성 정책 제거 판단 1, manifest 광고 수정 1.

**Phase26 완료가 뜻하는 것**: 정식 게이트가 현재 코드 전체(마일스톤 + phase 7~25)를 실행한다. 제외된 항목은 사유가 보이고, 통과하기 시작하면 게이트가 이를 알린다.

**뜻하지 않는 것**: 제품 자격, 설계 적합성, 외부 독립 검증. `DEFERRED`는 해결이 아니다.
