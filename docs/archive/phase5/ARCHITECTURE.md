...
각 신규 모듈은 기존 모듈처럼 `install...(target, { bridge })` 시그니처를 따른다. DOM 주입 지점은 `#topbar`/`#subbar`/`#palette`/`#propPanel`/신규 도크.

실행 경로는 다음처럼 고정한다.

1. 현재 index 브리지는 상태ful 화면 API(`analyzeModel`, `getCurrentModel`, `runPushover`, 보고서/설계 trace)를 제공한다.
2. `analyzeDynamics`, `runResponseSpectrum`, `estimateGlobalBucklingTrace`, `runModalSuperpositionTha`, `runNewmarkNlth`처럼 브리지에 직접 없는 core 함수는 `analysisRunners.js`가 `src/index.js` export를 import해서 호출한다.
3. 브리지에 새 메서드를 추가하는 경우는 "현재 index 상태를 읽거나 갱신해야 하는 화면 작업"으로 제한한다. 순수 해석 runner는 브리지 확장을 전제로 하지 않는다.
...
// model.analysisCases[i]
{
  ...
  status: 'not-run' | 'running' | 'ok' | 'failed' | 'stale',
  lastRun: { at, ok, message, resultRef, summary, modelSignature } | null,
}
```

`stale`은 저장된 결과가 현재 모델과 다르다는 런타임/저장 상태다. 결과 핸들은 유지하되, 사용자는 재실행해야 한다. `modelSignature`는 최소 구현에서는 node/member/load/loadCase/loadCombination 개수와 수정 시각/해시 중 가능한 값으로 시작하고, 충돌이 발견되면 별도 티켓으로 강화한다.

kind별 settings 계약 (specs/SPEC-A 상세). UI에서 쓰는 짧은 이름은 runner에서 현재 엔진 파라미터로 정규화한다:

| kind | settings 주요 필드 | 실행 엔진 함수 |
| --- | --- | --- |
| static | `{ combo, pDelta: bool }` | `analyzeModel` (case-scoped clone에서 `analysisSettings.includeGeometricStiffness` 적용) |
| modal | `{ modalModeCount, massSource }` (`modeCount` UI alias 허용) | `analyzeDynamics` |
| responseSpectrum | `{ modalModeCount, spectrum: { method, directions, dampingRatio, scale, points } }` | `analyzeDynamics`의 `dynamics.rsa` 또는 `runResponseSpectrum` |
| buckling | `{ referenceCombo, modeCount, referenceCompression? }` | `estimateGlobalBucklingTrace` |
| linearTha | `{ accelerations, dt, direction, dampingRatio }` (`record`는 runner에서 accelerations로 변환) | `runModalSuperpositionTha` |
| pushover | `{ direction, pattern, steps, targetDisplacement, controlNodeId, maxLoadFactor, referenceBaseShear }` | `runPushover` |
| nlth | `{ accelerations, dt, mass, stiffness, damping, yieldForce, postYieldRatio, rayleigh?, scaling? }` | `runNewmarkNlth` |

범위 주의:
- RSA 현 엔진 반환은 방향별 modal response와 `combined` 변위/참여율 중심이다. 밑면전단·층응답 표시는 Phase 5 runner/result layer가 후처리하거나 limitation으로 표기한다.
- pushover 현 엔진은 load-factor 경로와 목표변위 중단을 제공한다. UI의 displacement/arc-length 선택지는 기존 control trace를 노출하는 preliminary 옵션이며, full tangent equilibrium pushover로 과장하지 않는다.
- NLTH 현 엔진은 SDOF/bilinear Newmark trace다. 전체 frame NLTH가 아니라 "현재 preliminary NLTH trace UI"로 표시한다.
...
4. 모델이 바뀌면 실행 완료 케이스 상태는 `stale`로 표시되어 재실행을 유도한다 — 결과와 모델의 불일치 방지.
...
| 모듈 | 책임 | 금지 |
| --- | --- | --- |
| `src/core/analysisCase.js` | 케이스 스키마·검증·기본값 | DOM·엔진 실행 |
| `src/ui/indexAnalysisCenter.js` | 케이스 목록 UI, 실행 오케스트레이션 | 해석 이론 |
| `src/ui/analysisRunners.js` | kind→엔진 함수 매핑, 현재 엔진 파라미터 정규화, 결과 정규화 | DOM |
...
`analysisRunners.js`가 엔진과 UI를 잇는 유일한 지점이다 — kind가 늘어도 여기만 확장한다. 모델 스키마 반영은 `createModel`/`migrateModel`/`validateModel`/`schemaContract`까지 함께 수정한다.
...
| 계층 | 대상 | 방법 |
| --- | --- | --- |
| 스키마 | analysisCase 검증·migration·schemaContract·roundtrip | 단위 (`tests/p5-analysis-case.mjs`, 기존 `m1-schema` 보강) |
...
| PRD가 preliminary trace를 production-grade 해석으로 오해 | SPEC/결과/계산서에 preliminary·limitation 표기를 유지 |
