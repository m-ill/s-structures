...
| P3-T53 | `verification/framework/nonlinearBenchmarks.js` registers B1 Euler buckling and B2 large-displacement cantilever screening gates | Preliminary |
...
| P3-T56 | `src/nonlinear/pushover.js` and `pushoverFormal.js` provide a formal pushover result contract with control metadata, hinge events, previous-step hinge secant stiffness reduction, regression comparison summary, and B4/B5 benchmark traces | Preliminary |

M15 is exposed through the existing `getNonlinearAnalysisTrace` contract. The formal pushover contract is now available for agents and reports, assigned member-end hinges now feed tangent assembly corrections, and pushover steps can carry previous-step hinge stiffness degradation into the next analysis step. Full simultaneous hinge-controlled global nonlinear iterations and PMM/fiber behavior remain later Phase 3 work.
...
- `verification/framework/nonlinearBenchmarks.js`
