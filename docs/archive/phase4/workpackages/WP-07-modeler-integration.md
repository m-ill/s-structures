...
1. **스파이크 (1일)**: `src/ui/indexRuntimeAdapter.js`와 index.html 부트 코드를 읽고, (a) 전역(window) 의존 목록, (b) DOM id 의존 목록, (c) 초기화/해제 가능 여부를 `docs/archive/phase4/workpackages/WP-07-spike-notes.md`로 기록. 통합 방식 결정: **iframe 격리** vs **동일 문서 마운트** — 스파이크 결과로 확정하고 이 문서에 결정 기록.
...
- Recorded the integration decision in `docs/archive/phase4/workpackages/WP-07-spike-notes.md`.
