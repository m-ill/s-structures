# Phase 11 Work Packages

| WP | Milestone | 이름 | 상태 |
| --- | --- | --- | --- |
| [WP-00](WP-00-baseline-governance.md) | P11-M0 | Baseline·범위·governance | qualification-complete |
| [WP-01](WP-01-report-snapshot-verdict.md) | P11-M1 | ReportSnapshot·verdict | qualification-complete |
| [WP-02](WP-02-i18n-dual-render.md) | P11-M2 | 한·영 현지화·dual HTML | qualification-complete |
| [WP-03](WP-03-visual-capture-core.md) | P11-M3 | 결정론적 visual capture core | qualification-complete |
| [WP-04](WP-04-scene-evidence-embedding.md) | P11-M4 | 필수 scene·본문 embedding | qualification-complete |
| [WP-05](WP-05-executive-report-layout.md) | P11-M5 | 첫 페이지 결론·production layout | qualification-complete |
| [WP-06](WP-06-pdf-export-service.md) | P11-M6 | 원자적 dual-PDF service | qualification-complete |
| [WP-07](WP-07-product-agent-workflow.md) | P11-M7 | UI·Agent·export history | qualification-complete |
| [WP-08](WP-08-qualification-hardening.md) | P11-M8 | 시각·성능·보안 qualification | qualification-complete |
| [WP-09](WP-09-pilot-release.md) | P11-M9 | Office pilot·release gate | qualification-complete |

각 WP는 대응 마일스톤 전용 test, evidence, `/code-review high`, 전체 회귀와 milestone commit이
완료된 뒤에만 `complete`로 갱신한다.
