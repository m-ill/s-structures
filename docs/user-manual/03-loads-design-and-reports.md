...
`일반구조설계` 탭의 설계기준 입력 영역은 자동 하중 생성 전용이다. 입력값을 preview하면 모델에는 설계기준 상태만 저장되고, apply를 눌러야 실제 하중 entity가 생성된다.
...
### 일반구조설계 공통 PDF (2026-09-15)

설계 검토의 PDF 버튼과 WebMCP `export_report_pdf`는 `general-structural-review-v1`을 사용한다. 내장 SStructuresSans 글꼴·고정 A4·벡터 텍스트로 출력하므로 시스템 글꼴을 측정하지 않는다. 수치·NG·미검토·KDS 근거를 보존하고, 입력·결과·snapshot·폰트·템플릿·렌더러 버전을 반환한다. 이미지 PDF 대신 검색·복사 가능한 PDF다. 실제 Mac/Safari 종합 시험은 별도로 수행해야 한다.

위의 기존 한·영 데스크톱 출력은 별도 경로다. Windows라는 이유만으로 출력 자격을 부여하지 않으며, 어댑터 가용성(`READY`)과 실제 산출물 검사를 분리한다. 명시적인 `BLOCKED`는 유지한다. 공통 검토 PDF에는 데스크톱 어댑터가 필요하지 않다. 어떤 경로든 보고서 생성이 최종 구조설계 승인을 뜻하지 않는다.
