...
2. 실무 도면(또는 공개 CAD 샘플) 변환 → DXF 파이프라인 → 후보 생성 → 검토 UI 확정 → validation 통과 모델 → 탄성해석 ok까지 전 과정 기록: `verification/specs/IMPORT_FIELD_VALIDATION.md` DWG 장 (단계별 스크린샷/카운트).
...
3. 자동 검출 실행 → recall/precision/오차 리포트 생성 → `verification/evidence/validation/pointcloud-field/`.
4. 결과가 합성 벤치마크 대비 크게 낮으면 R5 프로토콜: 목표 재설정(보조 도구 포지셔닝) + `STATUS_AND_LIMITS` 문구 초안.

### Step 3. 성능 실측 (T19)
1. 합성 생성기로 1e7점 파일 생성 (커밋 금지 — 생성 스크립트만).
2. 로드+전처리 시간, 피크 메모리, 뷰어 프레임타임 실측 → `verification/evidence/validation/pointcloud-perf.json`.
...
`verification/specs/IMPORT_FIELD_VALIDATION.md` / 실측·성능 리포트 / fuzz 테스트 / 승격 커밋 3건.
