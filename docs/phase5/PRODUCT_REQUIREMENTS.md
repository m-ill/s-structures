...
4. 이어서 "응답스펙트럼" 케이스 추가 → 스펙트럼·방향 설정 → [실행] → 층응답 확인.
...
4. NLTH 케이스 추가 → 지진파 선택·scaling → [실행] → 시간이력 응답 검토.

### S4. 결과·계산서
1. 각 해석 케이스 결과를 3D(모드 형상/좌굴 모드/힌지 분포)와 차트(곡선/시간이력)로 확인.
...
| FR-05 | 응답스펙트럼 케이스 (스펙트럼/방향/조합법, 층응답) | A | P0 |
...
| FR-19 | Pushover 케이스 (방향/패턴/제어/목표) + 실행 | C | P0 |
| FR-20 | Capacity curve·힌지 상태 진전 뷰 | C | P0 |
| FR-21 | 성능점·사용비·성능수준(IO/LS/CP) 판정 | C | P1 |
| FR-22 | NLTH 케이스 (지진파/scaling/감쇠) + 실행 | C | P1 |
...
| Check | Pass 조건 |
| --- | --- |
| 명시적 해석 | 해석 센터에서 6종 해석 케이스를 만들고 실행해 결과 확인 (브라우저 실동작) |
