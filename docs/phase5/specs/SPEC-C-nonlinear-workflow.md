...
- model: `member.hinges = { i:{backbone, type}, j:{...} }`.
...
- 설정 DOM: `#ssPoDirection`(±x/±y), `#ssPoPattern`(triangular/uniform/mass), `#ssPoControl`(load/displacement/arcLength), `#ssPoSteps`, `#ssPoTarget`(목표변위), `#ssPoControlNode`.
- 실행: `bridge.runPushover({ direction, pattern, control, steps, targetDisplacement, controlNode })` (기존 엔진).
...
- DOM: `#ssPoCurve`(svg 컨테이너), 데이터는 결과 핸들 `curve:[{controlDisplacement, baseShear, state}]`.

### 힌지 상태 진전 뷰
- 스텝 슬라이더로 힌지 상태(elastic→yield→capping→residual) 색상 3D 표시.
- DOM: `#ssPoStepSlider`, 힌지 상태는 결과 핸들 `hingeEvents`/스텝별 상태.
...
- 실행: `bridge`를 통해 `runNewmarkNlth({ ... })` (기존 엔진).
- 결과: 시간이력 응답(변위/층전단/힌지 상태), 최대치 envelope. 시간이력 차트(M10)와 연동.
- scaling trace(설계 스펙트럼 맞춤 배율)를 결과에 표기.
