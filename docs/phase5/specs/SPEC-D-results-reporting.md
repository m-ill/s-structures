...
- 종류별 3D:
  ...
  | buckling | criticalLoadFactor/trace 표시. 모드 형상은 trace가 제공될 때만 |
...
- 자체 SVG 차트(zero-dependency):
  ...
  | 응답스펙트럼 | rsa spectrum·modal/combined 응답. 층응답은 후처리 가능 시 | `#ssChartSpectrum` |
...
- 해석 케이스 결과가 계산서 해당 장에 편입:
  ...
  | responseSpectrum | 지진해석 (modal/combined 응답, 후처리 가능 시 층응답·scaling) |
  | buckling | 좌굴검토 (criticalLoadFactor·referenceCompression·limitation) |
...
M9: 케이스 선택→3D 전환(모달 형상/힌지 분포, 좌굴은 trace 제공 범위 표시). 모드 슬라이더. 브라우저 실동작.
