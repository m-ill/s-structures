...
start: 2026-07-04
mission: 이미 구현된 해석·설계 엔진을 사용자가 화면에서 명시적으로 조작할 수 있는 전문 구조해석 프로그램(OpenSees/Perform-3D 급 워크플로)으로 완성한다.
...
| 벤치마크 | OpenSees(명시적 해석 케이스·요소/재료/해석 조합)와 Perform-3D(성능기반 GUI 워크플로)를 참고. 완전 명령형이 아니라 **명시적 해석 케이스 + GUI 워크플로**의 절충 |
