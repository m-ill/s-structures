...
Phase 2는 현재의 선형 탄성해석 제품 흐름을 안정적인 구조설계 플랫폼으로 키우는 단계다. 목표는 기능을 더 붙이기 전에 파일 위치, 문서 위치, API 계약, 검증 산출물을 분리해서 장기 개발이 가능한 구조를 만드는 것이다.
...
| P1 | 기준식 registry 확장 계획 | 하중, RC, steel, connection, foundation registry 분리안 작성 |
