import { buildFlatShellAllmanDkq } from './flatShellAllmanDkq.js';
import { buildSlabPlateDkq } from './slabPlateDkq.js';
import { buildWallMembraneQm6 } from './wallMembraneQm6.js';

export const SHELL_BATCH_VERSION = 'p10-m9-shell-soa-batch-v1';
...
export function buildShellSoaBatch(elements = [], nodeIndex = new Map()) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    for (let local = 0; local < 4; local += 1) nodeIndices[index * 4 + local] = lookupNode(nodeIndex, ids[local]);
  ...
  return { ok: true, version: SHELL_BATCH_VERSION, elementCount: count, typeCodes, nodeIndices, properties, matrixOffsets, dofOffsets, matrices, rows };
}
...
function buildByFormulation(input, formulation) {
  ...
  if (formulation === 'plate') return buildSlabPlateDkq(input);
  return buildFlatShellAllmanDkq(input);
}
function normalizeFormulation(value) { return value === 'membrane' ? 'membrane' : value === 'plate' ? 'plate' : 'shell'; }
