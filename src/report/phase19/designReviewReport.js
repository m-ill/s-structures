...
export const DESIGN_REVIEW_REPORT_VERSION = 'p21-m4-design-report-v2';
...
export function createDesignReviewReport(model, record, context = {}) {
  ...
  const header = ['designRunId','analysisRunId','comboId','memberId','category','checkId','status','ratio','demand','capacity','unit','expression','checkKey','x','reason','inputHash','reportSnapshotHash','projectId','summaryStatus','summaryCounts','maxDisplacement','maxEquilibriumResidual','buildHash','rulePackHash'];
  const csvRows = [header, ...review.checks.map(row => header.map(key => key === 'designRunId' ? record.designRunId : key==='reportSnapshotHash'?snapshot.reportSnapshotHash:key==='inputHash'?record.identity.inputHash:key==='projectId'?snapshot.project.id:key==='summaryStatus'?review.summary.status:key==='summaryCounts'?JSON.stringify(review.summary.counts):key==='maxDisplacement'?snapshot.analysis.maxDisplacement:key==='maxEquilibriumResidual'?snapshot.analysis.maxEquilibriumResidual:key==='buildHash'?record.identity.buildHash:key==='rulePackHash'?record.identity.rulePackHash:row[key] ?? ''))];
  ...
  return { version: DESIGN_REVIEW_REPORT_VERSION, snapshot, reports, artifactManifest,
    json, csv,
...
function render(snapshot, locale) {
  ...
  const rows = review.checks.map(row => `<tr><td>${e(row.memberId)}</td><td>${e(row.category)}<br>${e(row.checkId)}</td><td>${e(row.comboId)}<br>${e(row.analysisRunId)}</td><td>${e(row.status)}</td><td>${e(row.ratio)}</td><td>${e(row.demand)} / ${e(row.capacity)} (${e(row.unit)})</td><td>${e(row.expression)}${row.reason?`<br>${e(row.reason)}`:''}</td></tr>`).join('');
  return `<!doctype html><html lang="${locale}"><meta charset="utf-8"><title>${title}</title><style>body{font:13px Arial,"Malgun Gothic",sans-serif;color:#14283d;margin:32px}h1{font-size:24px}table{border-collapse:collapse;width:100%;table-layout:fixed}td,th{border:1px solid #c6d0dc;padding:6px;overflow-wrap:anywhere;text-align:left}thead{display:table-header-group}tr{break-inside:avoid}.identity{overflow-wrap:anywhere;color:#456} @media print{@page{size:A4 landscape;margin:12mm}body{margin:0;font-size:9px}}</style><h1>${title}</h1><p>${ko ? '예비 검토 · 검토 필요 · 최종 설계전달 불가' : 'Preliminary · Review required · Final design transfer blocked'}</p><p>${e(review.designRunId)} · ${e(review.summary.status)} · ${ko?'최대 검정비':'Maximum utilization'}: ${e(review.summary.maxUtilization)}</p><p>${e(snapshot.project.id)} · 최대 변위(m): ${e(snapshot.analysis.maxDisplacement)} · 평형 잔차: ${e(snapshot.analysis.maxEquilibriumResidual)}</p><p>검토 집계: ${e(JSON.stringify(review.summary.counts))} · 실패 대상: ${e(review.summary.failedMemberCount)} · 설명: ${e(review.summary.messageCount)}</p><p class="identity">Source: ${e(snapshot.provenance.sourceRevision)} · Build: ${e(snapshot.provenance.buildId)} · Rule: ${e(snapshot.provenance.rulePackHash)}<br>Snapshot: ${e(snapshot.reportSnapshotHash)}<br>Input: ${e(review.inputIdentity.inputHash)}<br>Analysis runs: ${e(review.sourceAnalysisRunIds.join(', '))}</p><p>${e(JSON.stringify(review.units))} · ${e(review.axes)} · ${e(review.signConvention)}</p><table><thead><tr>${(ko?['부재','검토','조합 / 해석 기록','상태','검정비','수요 / 내력','식']:['Member','Check','Combination / run','Status','Ratio','Demand / capacity','Expression']).map(x=>`<th>${x}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table><h2>${ko?'설명 및 입력 검토 메시지':'Messages'}</h2><ul>${(review.messages||[]).map(row=>`<li>${e(row.memberId)} · ${e(row.comboId)} · ${e(row.code)} · ${e(row.message)}</li>`).join('')}</ul><h2>${ko?'적용 범위와 미검토 항목':'Scope and unchecked items'}</h2><ul>${review.limitations.map(x=>`<li>${e(x)}</li>`).join('')}</ul><h2>${ko?'구현·규칙 출처':'Implementation and rule sources'}</h2><ul>${review.ruleSources.map(x=>`<li>${e(x.module)} · ${e(x.method)} · ${e(x.status)}</li>`).join('')}</ul></html>`;
}
