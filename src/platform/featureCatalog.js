  ...
  },
  {
    ...
    features: [
      ...
      feature('releases-offsets', '단부 해제와 강역(offset)', '부재 단부 모멘트 해제(핀), 단부 강역 길이 지정으로 clear span 설계 길이 반영.', {
        description: '단부 해제는 부재 i/j단의 모멘트 전달을 끊어 힌지 접합을 모델링한다(거셋 단순접합 보 등). 강역(end offset)은 보-기둥 접합부 크기만큼 단부를 강체로 처리해 유연 길이(clear span)를 줄이고, 설계 모듈에는 clear length가 전달된다.',
        howTo: [
        ...
        limits: ['부분 고정(회전 스프링 단부)은 미지원 — pin/rigid 2단'],
  ...
  },
  {
    ...
    features: [
      ...
      feature('pdelta', 'P-Delta (2차효과)', '축력-변위 반복 증폭 방식 P-Delta. 조합별 수렴/증폭 로그가 남는다.', {
        description: '중력 축력과 횡변위의 곱으로 생기는 2차 모멘트를 반복 재하로 수렴시킨다. 조합별 반복 횟수·증폭비·수렴 여부가 trace로 남고, 최대 증폭 한계를 넘으면 경고된다. 결과 화면에서 반복 스텝별 변형을 볼 수 있다.',
        howTo: [
          '탄성해석 리본의 P-Delta 토글(ssNativePDeltaToggle)을 켠다',
          '해석 실행 후 P-Delta 스텝 선택으로 반복 단계별 결과를 확인한다',
          '수렴 로그와 증폭비는 고급 탄성 trace에서 확인한다',
        ],
        limits: ['반복 증폭 방식 — 기하강성 기반 대변위 해석은 비선형(기하) 모드 사용'],
  ...
  },
  {
    ...
    features: [
      ...
      feature('result-overlays', '3D 결과 overlay', '변형도/부재력 다이어그램을 모델 위에 겹쳐 표시, P-Delta 스텝 뷰 포함.', {
        description: '해석 결과를 3D 모델 위에 겹쳐 그리는 표시 계층이다. 표시 모드(변형/다이어그램)와 옵션을 전환하고, P-Delta를 켰다면 반복 스텝별 상태를 넘겨 볼 수 있다.',
        howTo: [
          '결과 표시 상태에서 overlay 모드(setOverlayMode)와 옵션(setOverlayOption)을 전환한다',
          'P-Delta 스텝 뷰는 setOverlayPDeltaStep으로 단계를 이동한다',
