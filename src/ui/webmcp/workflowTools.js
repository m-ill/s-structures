...
export function createWorkflowTools({agent,bridge,tool,object,context,setView}) {
  ...
  const save=(kind,value)=>{if(handles.size>=128)fail('SESSION_HANDLE_LIMIT');const handle=`${kind}-${++sequence}`;handles.set(handle,{kind,value});return handle;};
  ...
  const tools=[
    ...
    tool('start_design_review','Calculate and record preliminary design from a stored plan; does not rerun analysis.',object({handle:id,requestId:id},['handle','requestId']),false,args=>once('design',args,()=>{const r=good(agent.startDesignReview({plan:get(args.handle,'design'),requestId:`webmcp-${args.requestId}`}));save('review',r.designRunId);return reviewSummary(r);})),
