...
export function createWorkflowTools({agent,bridge,tool,object,context,setView}) {
  ...
  function preview(args,commands) {
    ...
    return once('preview',args,()=>{
      ...
      return {ok:true,handle:save('changes',value),sourceIdentity:value.sourceIdentity,changed:value.changed,impact:value.impact,changeCount:value.changes.length,warnings:value.warnings.slice(0,20),warningCount:value.warnings.length,changes:page(value.changes,{})};
    });
  }
  const apply=args=>once('apply',args,()=>{const value=good(agent.applyDesignInputChanges(get(args.handle,'changes')));return {ok:true,changed:value.changed,transactionId:value.transactionId,inputRequestId:value.requestId,inputIdentity:value.inputIdentity,changeCount:value.changes.length,warningCount:value.warnings.length};});
  const tools=[
    ...
    tool('start_design_review','Calculate and record preliminary design from a stored plan; does not rerun analysis.',object({handle:id,requestId:id},['handle','requestId']),false,args=>once('design',args,async()=>{if(handles.size>=128)fail('SESSION_HANDLE_LIMIT');const r=good(await bridge.startDesignReviewAsync({plan:get(args.handle,'design'),requestId:`webmcp-${args.requestId}`}));if(!active)fail('SESSION_DISPOSED');save('review',r.designRunId);return reviewSummary(r);})),
    tool('cancel_design_review','Cancel the active design-review Worker; partial results are never recorded.',object({}),false,()=>bridge.cancelDesignReview()),
    ...
    tool('plan_report_export','Plan report creation from a current design record. Does not create a report or export files.',object({designRunId:id},['designRunId']),true,args=>{const r=good(agent.getDesignReview(args.designRunId));if(r.stale)fail('STALE_INPUT');return {ok:true,handle:save('report-plan',{id:r.designRunId,inputHash:identity().inputHash}),reportContract:REPORT_CONTRACT,formats:['html','json','csv'],automaticPdf:'canonical-vector-review; requires report artifact and bundled font'};}),
    ...
    tool('export_report_pdf','Generate and download the canonical general-structural-review-v1 PDF from an existing report. Uses embedded SStructuresSans, deterministic A4 layout and searchable text across operating systems. Preserves NG and unchecked items; does not authorize final design.',object({handle:id,requestId:id},['handle','requestId']),false,args=>once('pdf',args,()=>bridge.exportDesignReviewPdf(get(args.handle,'artifact')))),
