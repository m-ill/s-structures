...
import { PRACTICAL_DESIGN_TYPES, practicalInputSchema } from '../../modeling/practicalInputContract.js';
...
export const command={oneOf:[
 ...PRACTICAL_DESIGN_TYPES.map(practicalInputSchema),
 ...
 object({type:choice('load'),mode,value:object({id,type:choice('nodal','udl','nmoment','mmoment','point','udl-partial','trapezoid'),node:id,member:id,P:number,w:number,w1:number,w2:number,M:number,at:{type:'number',minimum:0,maximum:1},t:{type:'number',minimum:0,maximum:1},from:{type:'number',minimum:0,maximum:1},to:{type:'number',minimum:0,maximum:1},shape:choice('uniform','asc','desc'),dir:choice('+x','-x','+y','-y','+z','-z'),case:id,unit:choice('kN','kN/m','kN.m')},['id','type','dir','case','unit'])},['type','mode','value']),
 object({type:choice('load'),mode,value:object({id,type:choice('temperature','tgradient'),member:id,case:id,unit:choice('degC'),dT:number,dTtop:number,dTbot:number,h:{type:'number',exclusiveMinimum:0,maximum:1e4},hUnit:choice('m'),alpha:{type:'number',exclusiveMinimum:0,maximum:1},alphaUnit:choice('1/degC')},['id','type','member','case','unit'])},['type','mode','value']),
