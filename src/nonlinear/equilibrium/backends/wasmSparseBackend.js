...
import { symbolicFactor } from '../../../solver/sparse/symbolicFactor.js';
...
export async function createWasmSparseBackend({ wasmBytes, wasmUrl } = {}) {
  ...
  return {
    ...
    solve(matrix, rhs, options = {}) {
      return solveWithWasm(wasm, diagnosticLength, matrix, rhs, options, symbolicContext);
    },
  };
}
...
async function loadModule({ wasmBytes, wasmUrl }) {
  ...
  if (source === undefined) {
    const url = wasmUrl === undefined
      ? new URL('./phase8_solver.wasm', import.meta.url)
...
function solveWithWasm(wasm, diagnosticLength, matrix, rhs, options, symbolicContext) {
  let normalized;
  let normalizedRhs;
  let solveOptions;
  try {
