...
const rows=[
 ...
 // Phase29. A sixth element overrides the phase prefix; the rows above keep
 // the p25 default they were registered under.
 ['slab-strip',['slab-thickness','slab-flexure','slab-one-way-shear','slab-bar-spacing','slab-shrinkage-temperature'],'rc/slabStripReview.js',['142070','142020','142022','142050'],'one-way slab on a unit-width strip against supplied reinforcement; deflection-governed thickness (KDS 14 20 30) and the shrinkage ratio above fy 400MPa are not covered','p29'],
 ['slab-coefficient-method',['slab-two-way-moments'],'rc/slabCoefficientMethod.js',['142070'],'design moments and shear distribution for a rectangular panel on four edges from the KDS 14 20 70 appendix; produces demands only, and section strength, detailing and deflection remain separate checks','p29'],
 ['wall-section',['wall-thickness','wall-minimum-vertical-reinforcement','wall-minimum-horizontal-reinforcement','wall-bar-spacing','wall-reinforcement-layers','wall-axial-strength'],'rc/wallSectionReview.js',['142072','142022'],'wall minimum ratios, spacing, thickness and layer rules against supplied reinforcement; the practical design method axial strength equation (4.3-1) is an image in the captured text and is not implemented','p29'],
];
export const PRACTICAL_RULE_IMPLEMENTATIONS=Object.freeze(rows.map(([id,checkIds,module,documents,scope,phase='p25'])=>Object.freeze({id:`${phase}-${id}`,version:1,checkIds,module:`src/design/${module}`,scope,implementation:'PARTIAL_PROFILE',sources:getKcscRuleSources(documents).map(({id,edition,sha256})=>({id,edition,sha256})),independentlyVerified:false})));
